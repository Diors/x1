...
import com.xonesoft.x1.sso.core.util.CookieUtil;
...
public class SsoWebLoginHelper {
	 ...
	 * @param XoneUser
	 */
	public static void login(HttpServletResponse response, String sessionId, XoneSsoUser XoneUser, boolean ifRemember) {
	 ...
	 * @return
	 */
	public static XoneSsoUser loginCheck(HttpServletRequest request, HttpServletResponse response) {

		String cookieSessionId = CookieUtil.getValue(request, Conf.SSO_SESSIONID);

		// cookie user
		XoneSsoUser XoneUser = SsoTokenLoginHelper.loginCheck(cookieSessionId);
		if (XoneUser != null) {
			return XoneUser;
		}
		...
		XoneUser = SsoTokenLoginHelper.loginCheck(paramSessionId);
		if (XoneUser != null) {
			CookieUtil.set(response, Conf.SSO_SESSIONID, paramSessionId, false); // expire when browser close （client
																					// cookie）
			return XoneUser;
		}
